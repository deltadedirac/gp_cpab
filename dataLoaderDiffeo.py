...
def define_dataset_variablelength_seqs(path, alphabets):
		...
		alphabet = alphabets
		num_classes = len(alphabets)#np.unique(seqs_np).shape[0]
		...
		return prot_space2, identifiers, lengths, num_classes, alphabet
...
class datasetLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, **kwargs):
        ...
        if enable_var_seqs == True:
            self.prot_space, self.identifiers, self.lengths, self.num_classes, self.alphabet = define_dataset_variablelength_seqs(kwargs.get("pathBLAT_data"),alphabet)
