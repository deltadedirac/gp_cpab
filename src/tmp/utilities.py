...
def plot_logos_results(x1,ref_msa,x1_trans, alphabets, c2i,i2c,i2i,msa_num, path, loss_vals, modeflag, pathfolder):  
    ...
    #pdb.set_trace()
    msa_num = np.stack(msa_num).argmax(-1)
    logos =logomaker_plots( alphabets_logo, c2i, i2c, i2i, msa_num)
    dist = logos.generate_information_matrix('dist_'+path, pathfolder)
    return raw, ref, best, dist

def df_construction_aas(tmp, shape, columns):
    ff = np.vstack(tmp).reshape(-1,shape[1],shape[2])
    ff_final = np.average(ff,axis=0)
    prot_df =  pd.DataFrame(data = ff_final, columns=columns, index=[i for i in range(ff_final.shape[0])] )
    return prot_df


def plot_logos_probs(x1,ref_msa,x1_trans, alphabets, c2i,i2c,i2i,msa_num, path, loss_vals, modeflag, pathfolder):  
    from logomakers import logomaker_plots  

    alphabets_logo = [ i if i!='-' else 'X' for i in alphabets] 

    x1_logo_input = df_construction_aas([ x1.detach().numpy() ]*2000, x1.shape, alphabets_logo)

    x1_trans_logo_input = df_construction_aas([ x1_trans.detach().numpy() ]*2000, x1_trans.shape, alphabets_logo)
    ref_logo_input = df_construction_aas([ ref_msa.detach().numpy() ]*2000, ref_msa.shape, alphabets_logo)
    msa_num2 = df_construction_aas(msa_num, ref_msa.shape, alphabets_logo)

    #pdb.set_trace()
    raw = logomaker_plots.plotlogos(x1_logo_input,pathfolder,'original_'+path)
    ref = logomaker_plots.plotlogos(ref_logo_input,pathfolder,'ref_'+path)
    best = logomaker_plots.plotlogos(x1_trans_logo_input[['L','Q','R']],pathfolder,'transformed_'+path)
    dist = logomaker_plots.plotlogos(msa_num2[['L','Q','R']], pathfolder,'distr_'+path)
    #best = logomaker_plots.plotlogos(x1_trans_logo_input[['L','Q']],pathfolder,'transformed_'+path)
    #dist = logomaker_plots.plotlogos(msa_num2[['L','Q']], pathfolder,'distr_'+path)
