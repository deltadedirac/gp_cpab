...
class gp_cpab(Cpab):
    ...
    def transform_data(self, data, theta, outsize):
        ...
        self.grid_t_no_expansion = self.transform_grid(grid_no_expansion, -theta)

        #data_t = self.interpolate(data, grid_t, outsize)
        if self.interpolation_type == 'linear':
            data_t = self.interpolate(data, self.grid_t, outsize)
            return data_t
        else:       #data_t = self.interpolate_GP_1D_multitask(data, grid_t, outsize)
            '''data_t = self.interpolate_GP_1D_group_gps(data, grid_t, outsize)'''
            data_t, sampled_data = self.interpolate_GP_1D_multitask2(data, self.grid_t, outsize)
    ...
    # Problem size
        ...
        x = x * (width-1)
        # Do sampling
        x0 = torch.round(x).to(torch.int64)
        # Clip values
    ...
    def interpolate_GP_1D_multitask2(self,data, grid, outsize):
    # Problem size
        n_batch = data.shape[0]
        n_channels = data.shape[1]
        width = data.shape[2]
        ...
        pdb.set_trace()#pdb.set_trace()
        x = self.grid_t_no_expansion[:,0].flatten()
        self.cpabgrid = self.grid_t_no_expansion[:,0].flatten() * (width-1)
        # Scale to domain
        x = x * (width-1)
        '''JUST CHANGE IT LATER, VERY DIRTY!!!'''
        
        # Do sampling
        x0 = torch.round(x).to(torch.int64)
        # Clip values
        #x0 = torch.clamp(x0, 0, width-1)
        x0 = torch.clamp(x0, 0, grid.shape[2]-1)
        x = self.grid_t_no_expansion[:,0].flatten() * (grid.shape[2]-1)
        ...
        trans_data = self.predict_operation(grid[:,0].flatten() *(grid.shape[2]-1)) #* (width-1))
        trans_data_train = self.predict_operation(self.cpabgrid.flatten())
...
if __name__ == "__main__":
    ...
    if option == 'multitask':
        ...
        T.get_interpolation_inductive_points(x1, x1.float(), likelihood, channels, option = 'multitask')
    ...
    if os.path.isfile(path_preexist_model):
        ...
        for i in range(maxiter):
            ...
            if T.interpolation_type == 'linear':
                ...
                distribution_sampling(False,modeflag,x1_trans, sampled_data, msa_num)
